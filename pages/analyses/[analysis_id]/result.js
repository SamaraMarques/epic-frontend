...
const AnalysisResult = () => {
  ...
  const [enterpriseIndex, setEnterpriseIndex] = useState('');
  ...
  useEffect(() => {
    api
      ...
      .then((response) => {
        ...
        setEnterpriseIndex(defineClassificacao(response.data.enterprise.index));
      })
      .catch((err) => {
        console.log(err);
        router.push(`/enterprises`);
      });
  }, [token, router, analysis_id, setResult, setEnterpriseIndex]);
    ...
    <Box>
      ...
      <Stack m={3} direction="column">
        ...
        {result && (
          <Typography
            mt={3}
          >{`Empresa de nome ${result?.enterprise.name.replace(/^\w/, (c) =>
            c.toUpperCase(),
          )} apresentou índice de segurança ${
            result?.enterprise.index
          } implementado, considerado ${enterpriseIndex}`}</Typography>
        )}
